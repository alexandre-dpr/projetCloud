...
## Fin installation de cert-manager
  ...
  provisioner "local-exec" {
    ...
    command = "helm install ingress-controller ingress-nginx/ingress-nginx --set controller.nodeSelector.\"beta\\.kubernetes\\.io/os\"=linux --set defaultBackend.nodeSelector.\"beta\\.kubernetes\\.io/os\"=linux --set controller.service.type=LoadBalancer"
  }
}